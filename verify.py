# partial refer to https://github.com/openai/prm800k/blob/main/prm800k/grading/grader.py
import re
import json
...
def is_correct(ans, label_answer):
    if not ans or ans is None:  
        return False
    if type(ans) is int or type(ans) is float or ans.replace('.', '').isdigit():
        ans = float(ans.rstrip('.')) # remove "..." like "79.999..."
    else:
        ans = process_string(ans)
    if type(label_answer) is int or type(label_answer) is float or label_answer.replace('.', '').isdigit():
        label_answer = float(label_answer)
    else:
        label_answer = process_string(label_answer)
    if type(label_answer) == type(ans) and type(label_answer) is float:
        return ans == label_answer
    else:
        return str(label_answer) == str(ans)
